...

{
  ...
  health_overview_stat(
    ...
    overview_stat(
      ...
      decimals=0,
      unit='none',
      expr=std.format('sum(up{job=~"%s"})', cfg.job),
    ) { gridPos: { w: 6, h: 3 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  memory_used_stat(
    ...
    overview_stat(
      ...
      decimals=2,
      unit='bytes',
      expr=std.format('sum(tnt_slab_arena_used{job=~"%s"})', cfg.job),
    ) { gridPos: { w: 3, h: 3 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  memory_reserved_stat(
    ...
    overview_stat(
      ...
      decimals=2,
      unit='bytes',
      expr=std.format('sum(tnt_slab_quota_size{job=~"%s"})', cfg.job),
    ) { gridPos: { w: 3, h: 3 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  space_ops_stat(
    ...
    overview_stat(
      ...
      decimals=2,
      unit='ops',
      expr=std.format('sum(rate(tnt_stats_op_total{job=~"%s"}[$__rate_interval]))', [cfg.job]),
    ) { gridPos: { w: 4, h: 5 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  http_rps_stat(
    ...
    overview_stat(
      ...
      decimals=2,
      unit='reqps',
      expr=std.format('sum(rate(http_server_request_latency_count{job=~"%s"}[$__rate_interval]))', [cfg.job]),
    ) { gridPos: { w: 4, h: 5 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  net_rps_stat(
    ...
    overview_stat(
      ...
      decimals=2,
      unit='reqps',
      expr=std.format('sum(rate(tnt_net_requests_total{job=~"%s"}[$__rate_interval]))', [cfg.job]),
    ) { gridPos: { w: 4, h: 5 } }
  else if cfg.type == variable.datasource_type.influxdb then
    error 'InfluxDB target is not supported yet',
  ...
  local cartridge_issues(
    ...
    if cfg.type == variable.datasource_type.prometheus then
      prometheus.target(
        expr=std.format('tnt_cartridge_issues{job=~"%s",alias=~"%s",level="%s"}', [cfg.job, cfg.filters.alias, level]),
        legendFormat='{{alias}}',
      )
    else if cfg.type == variable.datasource_type.influxdb then
      influxdb.target(
